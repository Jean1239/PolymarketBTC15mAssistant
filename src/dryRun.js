...
export function createDryRunSimulator15m(csvPath, tradingConfig = {}) {
  const config = {
    ...
    entryMinMarketPrice: tradingConfig.entryMinMarketPrice ?? 0.50,
    entryMaxMarketPrice: tradingConfig.entryMaxMarketPrice ?? 0.58,
    btcVsPtbMinAbsUsd: tradingConfig.btcVsPtbMinAbsUsd ?? 5,
    blockedHoursUtc: tradingConfig.blockedHoursUtc ?? [0, 8, 9, 11, 17, 18, 19, 21, 22],
    ...
    highConvictionMultiplier: tradingConfig.highConvictionMultiplier ?? 2,
...
export function createDryRunSimulator5m(csvPath, tradingConfig = {}) {
  const config = {
    ...
    disableTimeDecay: tradingConfig.disableTimeDecay ?? true,
    entryMinMarketPrice: tradingConfig.entryMinMarketPrice ?? 0.50,
    entryMaxMarketPrice: tradingConfig.entryMaxMarketPrice ?? 0.52,
    btcVsPtbMinAbsUsd: tradingConfig.btcVsPtbMinAbsUsd ?? 0,
    blockedHoursUtc: tradingConfig.blockedHoursUtc ?? [2, 3, 4, 6, 10, 16, 19, 20],
