...
- **edge5m.js** — Re-exports `computeEdge`; `decide5m` uses 5m-tuned phases (EARLY >3m / MID >1.5m / LATE) and lower thresholds. Early-window gate: rejects entry when `remainingMinutes < minTimeLeftMin` (passed from `CONFIG.trading.entryMinTimeLeftMin`, default 4), returning `NO_TRADE` reason `too_late_lt_<n>m` — restricts entries to the first ~minute of the market. OFI alignment filter: rejects entry when `ofi_1m` contradicts the chosen direction (`ofi_1m < -0.05` for UP, `ofi_1m > 0.05` for DOWN), returning `NO_TRADE` with reason `ofi_conflict`. Previously required both HA and OFI to disagree; OFI alone is now sufficient since it is the primary 5m signal. *(5m)*
...
| `TRADE_SLIPPAGE_TOLERANCE_PCT` | `0.02` | Max fractional drift between the sim's decision price and the live bestBid before a real SELL is skipped. Only applies to SELL — BUY now uses a sim-anchored limit price instead (see `TRADE_TAKER_BUFFER`). `0.02` = 2%. |
| `TRADE_TAKER_BUFFER` | `0.05` | Absolute price buffer used when posting the FAK limit. On BUY: limit = `simDecisionPrice + takerBuffer` (anchored to the sim's decision price, not live `bestAsk`, so the cap does not age between snapshot and order-send and the FAK kills only when filling would be -EV at the sim's probability). On SELL: limit = `bestBid - takerBuffer`. CLOB still matches at the best available ask ≤ limit, so a normal book fills at simDecisionPrice or better; the buffer is the maximum slippage tolerated above that. Lowered 0.10 → 0.05 after 196-trade real run showed 23% of fills landing above the configured `entryMaxMarketPrice` band. The executor also enforces a pre-flight cap that aborts BUY when `simDecisionPrice > entryMaxMarketPrice` (sim/exec drift safety net — the sim already gates on this band). The earlier `simDecisionPrice + takerBuffer > entryMax` form was wrong: with `entryMax_5m=0.52` + `buffer=0.05` every 5m signal was unreachable. Partial fills above the band remain logged post-fill as belt-and-suspenders. `0.05` = 5¢. |
...
| `TRADE_ENTRY_MAX_PRICE_5M` | `0.51` | 5m: maximum market price of chosen side to allow entry. Lowered 0.52→0.51 — fee-correct 280-trade run (`bc1547ba`) showed the 0.52 band toxic (n=121, WR 43.8%, net −$23.14, negative all 3 days; breakeven WR at 0.52 is 53.8%). Dropping it lifts net +$2.56 → +$25.71. Earlier 0.60→0.52 cut for the same reason. |
| `TRADE_ENTRY_MIN_TIME_LEFT_MIN_5M` | `4` | 5m: minimum minutes left in the market to allow a new entry — enforced in `decide5m` (returns `NO_TRADE` reason `too_late_lt_<n>m`). Restricts entries to the first ~minute of the 5m window: entries with ≥4 min left held to settlement won 58% (+$19.6); entries with <4 min left lost (3-4 min bucket WR 41.7%, −$13.2). With the 0.51 cap, net rises to +$29.66 at 65.2% WR. Set to `0` to disable. |
