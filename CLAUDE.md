...
| `TRADE_TAKER_BUFFER` | `0.10` | Absolute price buffer used when posting the FAK limit. On BUY: limit = `simDecisionPrice + takerBuffer` (anchored to the sim's decision price, not live `bestAsk`, so the cap does not age between snapshot and order-send and the FAK kills only when filling would be -EV at the sim's probability). On SELL: limit = `bestBid - takerBuffer`. CLOB still matches at the best available ask ≤ limit, so a normal book fills at simDecisionPrice or better; the buffer is the maximum slippage tolerated above that. `0.10` = 10¢. |
